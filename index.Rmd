...
> Kernidee jeder Mehrebenenmodellierung ist, der Nestung der Daten dadurch Rechnung zu tragen, dass Effekte/Parameter sich zwar über die Cluster hinweg variieren können (Random Effects) aber simultan auch eine übergreifende Gemeinsamkeit modelliert wird (fixed effect). 
...
$$ICC(1) = \rho=\frac{\sigma_{u_{0}}^{2}}{\sigma_{u_{0}}^{2}+\sigma_{e}^{2}}$$
Wobei $\sigma_{u_{0}}^{2}$ und $\sigma_{e}^{2}$ die Residuen auf Ebene 1 bzw. Ebene 2 des Interecept Only Models darstellen, in denen jeweils i und j die Indices für Mirco- und Marcoeinheiten darstellen.
$$Y_{i j}=\beta_{0 j}+e_{i j}$$
$$\beta_{0 j}=\gamma_{00}+u_{0 j}$$
$$Y_{i j}=\gamma_{00}+u_{0 j}+e_{i j}$$
...
## Einfaches Single-Level Modell mit Multi-Level Modell vergleichen
