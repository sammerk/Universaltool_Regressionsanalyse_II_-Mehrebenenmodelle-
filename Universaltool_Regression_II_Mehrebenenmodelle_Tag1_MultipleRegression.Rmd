...
Diejenigen, die nicht `r fontawesome::fa(name = "r-project")` nutzen wollen, können den Datensatz `r xfun::embed_file("data/data_kidiq.sav", "data_kidiq.sav", "hier")` im SPSS-Format herunterladen.
...
#### Diagnostik der Voraussetzungen für die Inferenzstatistik
Da Regressionsmodelle mit Dummyvariablen eben auch Regressionsmodelle sind `r emo::ji("smiley")` gilt es auch bei diesen *dieselben* Voraussetzungen zu prüfen:
```{r}
check_model(mod03)
```
